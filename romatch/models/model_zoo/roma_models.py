...
def roma_model(
    ...
    **kwargs,
    ...
    # DINOv3 모델 확인 (patch_size 결정용)
    dinov3_model = kwargs.get('dinov3_model', None)
    patch_size = 16 if dinov3_model else 14
    ...
    # DINOv3는 patch_size=16, DINOv2는 patch_size=14
    assert resolution[0] % patch_size == 0, f"Needs to be multiple of {patch_size} for backbone"
    assert resolution[1] % patch_size == 0, f"Needs to be multiple of {patch_size} for backbone"
    ...
    # dinov3_model 파라미터 처리
    dinov3_model = kwargs.pop('dinov3_model', None)
    ...
    if weights is not None:
        # DINOv3 사용 시 프로젝션 레이어가 추가되므로 strict=False 사용
        if dinov3_model is not None:
            missing_keys, unexpected_keys = matcher.load_state_dict(weights, strict=False)
            if missing_keys:
                logger.warning(f"Missing keys (will be randomly initialized): {missing_keys}")
            if unexpected_keys:
